...
export default function ShopPage() {
  ...
  return (
    <div className="flex flex-col overflow-x-hidden">
      ...
      {/* 商品一覧 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 max-w-full">
        {products.length === 0 ? (
          <p className="col-span-full py-8 text-center text-sm text-gray-400">商品が登録されていません</p>
        ) : (
            ...
            return (
              <Link key={product.id} href={`/shop/${product.id}`}>
                <Card className="overflow-hidden transition-colors hover:bg-gray-50">
                  {/* スマホ: 横並びリスト表示 */}
                  <div className="flex gap-3 p-2 sm:hidden">
