...
export default function GameEditPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [gameData, lineupData, playersData] = await Promise.all([
          getGame(gameId),
        ...
        setFormData({
          ...
          score_team: gameData.score_team != null ? String(gameData.score_team) : "",
          score_opponent: gameData.score_opponent != null ? String(gameData.score_opponent) : "",
          notes: gameData.notes ?? "",
        });

        // イニングスコアの初期値をセット
        if (gameData.inning_scores && gameData.inning_scores.length > 0) {
          const inningCount = gameData.inning_scores.length;
          setInnings(inningCount);
          setInningScores(
            gameData.inning_scores.map((is: InningScore) => ({
              inning: is.inning,
              score_team: String(is.score_team),
              score_opponent: String(is.score_opponent),
            }))
          );
        }

        // ラインナップの初期値をセット
        const starters = lineupData
          .filter((l) => l.is_starter)
          .sort((a, b) => (a.batting_order ?? 99) - (b.batting_order ?? 99));

        const newLineup: LineupEntry[] = Array.from({ length: 9 }, (_, i) => {
    ...
    load();
  }, [currentTeam, teamLoading, gameId]);
    ...
    try {
      ...
      const parsedInningScores: InningScore[] | undefined = hasInningScores
        ? inningScores.map((is) => ({
            ...
            score_opponent:
              is.score_opponent !== "" ? parseInt(is.score_opponent) || 0 : 0,
          }))
        : undefined;
      ...
      // ラインナップ更新: 既存を削除して再登録
      await deleteGameLineups(gameId);
  ...
  if (!canEditGame) return <ErrorDisplay message="試合編集の権限がありません" />;
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-2 border-b border-gray-200 bg-white px-4 py-3">
        ...
        <h2 className="text-base font-bold text-gray-900">試合結果編集</h2>
      </div>
                    ...
                    {inningScores.map((is, i) => (
                      <td key={is.inning} className="px-0.5 py-1">
                        <input
                          ...
                          onChange={(e) =>
                            updateInningScore(i, "score_opponent", e.target.value)
                          }
                        />
                      </td>
                    ))}
