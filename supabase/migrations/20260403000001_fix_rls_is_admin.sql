-- RLSポリシーに is_admin 条件を追加
-- permission_group チェックがあるが is_admin = true が漏れているポリシーを一括修正
-- サイト管理者（is_admin=true）が全機能にアクセスできるようにする
...
CREATE POLICY "teams_update_admin" ON teams FOR UPDATE USING (
  id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group = 'director' OR is_admin = true))
);
...
CREATE POLICY "team_members_insert_admin" ON team_members FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group = 'director' OR is_admin = true))
  OR user_id = auth.uid()
);
...
CREATE POLICY "players_insert_admin_manager" ON players FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'coach') OR is_admin = true))
);
...
CREATE POLICY "players_insert_admin_manager_parent" ON players FOR INSERT WITH CHECK (
  ...
  OR team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid() AND (permission_group = 'parent' OR is_admin = true)
  )
);
...
CREATE POLICY "user_children_insert" ON user_children FOR INSERT WITH CHECK (
  user_id = auth.uid()
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group = 'director' OR is_admin = true))
);
...
CREATE POLICY "events_insert" ON events FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
);
...
CREATE POLICY "posts_insert" ON posts FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach', 'publicity') OR is_admin = true))
);
...
CREATE POLICY "player_goals_insert" ON player_goals FOR INSERT WITH CHECK (
  ...
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'coach') OR is_admin = true))
);
...
CREATE POLICY "best_plays_insert" ON best_plays FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'coach', 'publicity') OR is_admin = true))
);
...
CREATE POLICY "albums_insert" ON albums FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'publicity') OR is_admin = true))
);
...
-- album_photos
-- ============================================================
-- album_photos INSERT: 全チームメンバーがアクセス可能のため is_admin 追加不要（スキップ）
...
CREATE POLICY "album_photos_delete" ON album_photos FOR DELETE USING (
  ...
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'publicity') OR is_admin = true))
);
...
CREATE POLICY "fee_settings_insert" ON fee_settings FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'treasurer') OR is_admin = true))
);
...
CREATE POLICY "inter_team_messages_select" ON inter_team_messages FOR SELECT USING (
  from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president') OR is_admin = true))
  OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president') OR is_admin = true))
);
...
CREATE POLICY "inter_team_messages_insert" ON inter_team_messages FOR INSERT WITH CHECK (
  sender_id = auth.uid()
  AND from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president') OR is_admin = true))
);
...
CREATE POLICY "inter_team_messages_update" ON inter_team_messages FOR UPDATE USING (
  to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president') OR is_admin = true))
);
...
CREATE POLICY "match_requests_select" ON match_requests FOR SELECT USING (
  from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
  OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
);
...
CREATE POLICY "match_requests_insert" ON match_requests FOR INSERT WITH CHECK (
  ...
  AND from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
);
...
CREATE POLICY "match_request_dates_select" ON match_request_dates FOR SELECT USING (
  match_request_id IN (
    ...
    WHERE from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
       OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
  )
);
...
CREATE POLICY "notifications_insert" ON notifications FOR INSERT WITH CHECK (
  team_id IS NULL
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND (permission_group IN ('director', 'vice_president', 'coach') OR is_admin = true))
);

-- ============================================================
-- player_cards (テーブルが存在しない可能性があるため例外処理で囲む)
-- ============================================================
DO $$ BEGIN
