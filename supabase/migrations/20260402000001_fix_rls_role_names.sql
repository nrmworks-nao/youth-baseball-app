-- RLSポリシーの旧ロール名を新ロール名に修正
-- system_admin, team_admin → director
-- manager → coach
-- グローバルshop系は is_admin フラグに変更
...
CREATE POLICY "teams_update_admin" ON teams FOR UPDATE USING (
  id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);
...
CREATE POLICY "team_members_insert_admin" ON team_members FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
  OR user_id = auth.uid()
);
...
CREATE POLICY "players_insert_admin_manager" ON players FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach'))
);
...
CREATE POLICY "user_children_insert" ON user_children FOR INSERT WITH CHECK (
  user_id = auth.uid()
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);
...
CREATE POLICY "events_insert" ON events FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
);
...
CREATE POLICY "posts_insert" ON posts FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach', 'publicity'))
);
...
CREATE POLICY "player_measurements_select_admin_manager" ON player_measurements FOR SELECT USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach'))
);
...
CREATE POLICY "player_goals_insert" ON player_goals FOR INSERT WITH CHECK (
  ...
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach'))
);
...
CREATE POLICY "best_plays_insert" ON best_plays FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach', 'publicity'))
);
...
CREATE POLICY "albums_insert" ON albums FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'publicity'))
);
...
CREATE POLICY "album_photos_insert" ON album_photos FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'publicity', 'parent'))
);
...
CREATE POLICY "album_photos_delete" ON album_photos FOR DELETE USING (
  ...
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'publicity'))
);
...
CREATE POLICY "fee_settings_insert" ON fee_settings FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'treasurer'))
);
...
-- ============================================================
-- shop_categories (グローバル管理 → is_admin フラグ使用)
-- ============================================================
...
-- ============================================================
-- shop_products (グローバル管理 → is_admin フラグ使用)
-- ============================================================
...
-- ============================================================
-- shop_product_images (グローバル管理 → is_admin フラグ使用)
-- ============================================================
...
-- ============================================================
-- shop_product_links (グローバル管理 → is_admin フラグ使用)
-- ============================================================
...
CREATE POLICY "team_profiles_insert" ON team_profiles FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);

DROP POLICY IF EXISTS "team_profiles_update" ON team_profiles;
CREATE POLICY "team_profiles_update" ON team_profiles FOR UPDATE USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);
...
CREATE POLICY "team_line_config_select" ON team_line_config FOR SELECT USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);

DROP POLICY IF EXISTS "team_line_config_insert" ON team_line_config;
CREATE POLICY "team_line_config_insert" ON team_line_config FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);

DROP POLICY IF EXISTS "team_line_config_update" ON team_line_config;
CREATE POLICY "team_line_config_update" ON team_line_config FOR UPDATE USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group = 'director')
);
...
CREATE POLICY "inter_team_messages_select" ON inter_team_messages FOR SELECT USING (
  from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president'))
  OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president'))
);

DROP POLICY IF EXISTS "inter_team_messages_insert" ON inter_team_messages;
CREATE POLICY "inter_team_messages_insert" ON inter_team_messages FOR INSERT WITH CHECK (
  sender_id = auth.uid()
  AND from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president'))
);

DROP POLICY IF EXISTS "inter_team_messages_update" ON inter_team_messages;
CREATE POLICY "inter_team_messages_update" ON inter_team_messages FOR UPDATE USING (
  to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president'))
);
...
CREATE POLICY "match_requests_select" ON match_requests FOR SELECT USING (
  from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
  OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
);
...
CREATE POLICY "match_requests_insert" ON match_requests FOR INSERT WITH CHECK (
  ...
  AND from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
);
...
CREATE POLICY "match_request_dates_select" ON match_request_dates FOR SELECT USING (
  match_request_id IN (
    SELECT id FROM match_requests
    WHERE from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
       OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
  )
);

DROP POLICY IF EXISTS "match_request_dates_insert" ON match_request_dates;
CREATE POLICY "match_request_dates_insert" ON match_request_dates FOR INSERT WITH CHECK (
  match_request_id IN (
    SELECT id FROM match_requests
    WHERE from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
  )
...
CREATE POLICY "match_request_dates_update" ON match_request_dates FOR UPDATE USING (
  match_request_id IN (
    SELECT id FROM match_requests
    WHERE from_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
       OR to_team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
...
CREATE POLICY "head_to_head_records_insert" ON head_to_head_records FOR INSERT WITH CHECK (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach'))
);

DROP POLICY IF EXISTS "head_to_head_records_update" ON head_to_head_records;
CREATE POLICY "head_to_head_records_update" ON head_to_head_records FOR UPDATE USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'coach'))
);
...
CREATE POLICY "notifications_insert" ON notifications FOR INSERT WITH CHECK (
  team_id IS NULL
  OR team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid() AND permission_group IN ('director', 'vice_president', 'coach'))
);

-- ============================================================
-- 招待・メンバー登録関連のポリシー修正
-- (20260331000002_rls_invite_member_registration.sql で作成されたもの)
-- ============================================================

-- team_members: vice_president も UPDATE 可能
DROP POLICY IF EXISTS "team_members_update_admin_vp" ON team_members;
CREATE POLICY "team_members_update_admin_vp" ON team_members FOR UPDATE USING (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'vice_president')
  )
);

-- team_members: vice_president も DELETE 可能
DROP POLICY IF EXISTS "team_members_delete_admin_vp" ON team_members;
CREATE POLICY "team_members_delete_admin_vp" ON team_members FOR DELETE USING (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'vice_president')
...
-- teams: vice_president も更新可能
DROP POLICY IF EXISTS "teams_update_admin_vp" ON teams;
CREATE POLICY "teams_update_admin_vp" ON teams FOR UPDATE USING (
  id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'vice_president')
...
-- players: admin/coach/parent がINSERT可能
DROP POLICY IF EXISTS "players_insert_admin_manager_parent" ON players;
CREATE POLICY "players_insert_admin_manager_parent" ON players FOR INSERT WITH CHECK (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'coach')
  )
  OR team_id IN (
    SELECT team_id FROM team_members
...
-- players: admin/coach/vice_president/parent がUPDATE可能
DROP POLICY IF EXISTS "players_update_admin_manager_parent" ON players;
CREATE POLICY "players_update_admin_manager_parent" ON players FOR UPDATE USING (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'coach', 'vice_president')
  )
  OR id IN (
...
-- ============================================================
-- player_cards (20260331000005_phase4_schema_alignment.sql で作成)
-- ============================================================
DROP POLICY IF EXISTS "player_cards_insert" ON player_cards;
CREATE POLICY "player_cards_insert" ON player_cards FOR INSERT WITH CHECK (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'coach')
...
CREATE POLICY "player_cards_update" ON player_cards FOR UPDATE USING (
  team_id IN (
    SELECT team_id FROM team_members
    WHERE user_id = auth.uid()
      AND permission_group IN ('director', 'coach')
